...
- **政策区块**：每次模型请求都携带简短的政策块，点名已勾选角色及各自适用时机（advisor 决策点、reviewer 宣称完成前、designer 本轮首次写文件之后的下一步）。自动咨询模式为 `off | remind | required`（默认 `remind`）。`required` 是更硬的提醒（政策 + 写工具日志）；本插件的 inject 面没有 DSH 预执行钩子，并不能真正拦住 Write。
...
- **通用** —— CLI 路径、默认模型（完整目录：跟随 CLI 默认、最新别名、带版本全名如 `claude-opus-5`——直接取自 CLI 本体）、推理等级（`--effort`：低/中/高/超高/最大）、回退模型、单次超时、CLI 内最大轮数、并行会诊上限、单次咨询美元上限、附加 CLI 参数（走允许清单；`--settings` 会被拒绝）。
- **角色工作区** —— 新增 / 编辑 / 删除角色，每个角色含名称、显示名、用途说明、提示词、专属模型、专属回退与专属推理等级。角色开关为 omp 风格停用：保留在册但离开工具枚举，重新启用即恢复。
- **自动咨询** —— 默认勾选集、每角色每会话调用上限，以及触发模式（`off | remind | required`）；输入条开关按会话覆盖勾选集。
