 ...
 *
 * The plugin promises that a consultation never gains write/execute or
 * unsupervised capabilities, never loses the JSON result protocol and never
 * has its role contract replaced. Read scope may widen only through the
 * explicitly documented user-owned `--add-dir` exception below.
...
/**
 * Flags a consultation may legitimately carry. With one deliberate exception,
 * each is either strictly narrowing (it can only remove capability) or purely
 * operational (it cannot change the role contract, the tool set, the permission
 * surface or the output protocol).
 *
 * The exception is `--add-dir`, which *widens* what the consultant may read,
 * beyond the workspace the plugin otherwise confines it to. It stays allowed
 * because consulting about a repo that depends on a sibling checkout is a real
 * need and the setting is the user's own — but it is the one entry here that
 * grants rather than removes, so treat additions to this table sceptically.
 *
...
const REFUSED = {
  ...
  '--agents': 'redefines the agent the role contract is written for',
  '--agent': 'redefines the agent the role contract is written for',
  ...
  '--safe-mode': 'applied automatically when the installed CLI supports it',
  '--system-prompt': 'would replace the role contract',
  '--append-system-prompt': 'the role contract owns this flag',
  '--system-prompt-file': 'would replace the role contract',
  '--append-system-prompt-file': 'the role contract owns this flag',
  ...
  '--strict-mcp-config': 'applied automatically when the installed CLI supports it — it is what bounds the MCP tool surface',
...
const REFUSED_BOOLEAN = new Set([
  ...
  '--forward-subagent-text', '--verbose', '--print', '--no-session-persistence', '--strict-mcp-config',
