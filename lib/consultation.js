...
import { buildSystemPrompt, roleOutputKind } from './roles.js'
...
import { attachEnvelope, envelopeSchemaForRole } from './envelope.js'
...
/**
 * Request fields beyond question/context that change what the consultant is
 * actually asked — either because they reach argv (`model`, `effort`) or
 * because they become packet sections. A joined caller receives the *other*
 * call's answer, so anything here that is missing from the key is a silent
 * wrong answer, not merely a lost optimisation. Add to this list whenever a
 * new request field reaches `run()`.
 */
const SHAPING_FIELDS = [
  'model', 'effort', 'objective', 'successCriteria', 'constraints',
  'currentAttempt', 'verification', 'unknowns', 'maxPacketBytes',
]

/**
 * Fingerprint: session + role + normalized question + material digest + phase,
 * where the material digest covers the context *and* every shaping field.
 *
 * @param {object} input
 * @param {object} [input.shape] - the request, read for `SHAPING_FIELDS` only.
 */
export function consultationFingerprint({ sessionId = '', role, question, context, phase, shape = null }) {
  const material = createHash('sha256').update(normalizeText(context))
  for (const field of SHAPING_FIELDS) {
    // Field names are interleaved with the values, so two different shapes
    // cannot serialise to the same string by concatenation.
    material.update(`\u0000${field}\u0000${normalizeText(shape?.[field])}`)
  }
  return [sessionId, role, normalizeText(question), material.digest('hex').slice(0, 16), phase ?? 'tool'].join('\n')
}


function resolveConsultantModels(role, req, settings) {
  const primary = pick(req.model, role.model, settings.model)
  let fallbackModel = pick(role.fallbackModel, settings.fallbackModel)
  if (fallbackModel === primary) fallbackModel = ''
...
/**
 * Status-store adapters. The dock is pure instrumentation: a missing,
 * broken or throwing store must never change a consultation result, a ledger
 * settlement or a slot release.
 */
function statusBegin(status, input) {
  try {
    return typeof status?.begin === 'function' ? status.begin(input) : null
  } catch {
    return null
  }
}

function statusUpdate(entry, patch) {
  if (entry === null || entry === undefined) return
  try { entry.update?.(patch) } catch { /* lost card, never a lost consultation */ }
}

function statusFinish(entry, phase, fields) {
  if (entry === null || entry === undefined) return
  try { entry.finish?.(phase, fields) } catch { /* lost card, never a lost consultation */ }
}
 ...
 *        (`{ begin(input): handle|null }`); instrumentation only.
 */
export function createConsultationService({ settings, ledger = null, env = null, runner = runClaudeConsult, gate = null, status = null } = {}) {
   ...
   *
   * Identical *in-flight* calls join one spawn; a changed context digest is a
   * different key. There is deliberately no post-success cooldown: reusing a
   * finished answer would let a second tool call skip the attempt ledger, so
   * sequential repeats still pay the cap. See docs/plan/p1-review.md.
   */
  async function consult(req) {
    ...
    // Fingerprint the resolved model, not the raw request: role/global defaults
    // must not let two materially different calls join one spawn.
    const picked = resolveConsultantModels(role, req, settings)
    ...
    const key = consultationFingerprint({
      sessionId, role: role.name, question, context, phase: source,
      shape: { ...req, model: picked.primary },
    })
    const pending = inflightByKey.get(key)
    if (pending !== undefined) return pending

    const work = perform({ req, role, question, sessionId, source, picked, effort })
      .finally(() => { inflightByKey.delete(key) })
    inflightByKey.set(key, work)
    return work
  }

  async function perform({ req, role, question, sessionId, source, picked, effort }) {
    ...
    // Begin the dock card after de-duplication and the abort-at-entry check,
    // so one physical run is one card and a cancelled-before-start call never
    // paints one. The real store refuses blank sessions and the connection test.
    const entry = statusBegin(status, { sessionId, source, role: role.name, question })
    statusUpdate(entry, {
      model: { requested: picked.primary, fallback: picked.fallbackModel },
      effort,
    })
    ...
    if (ledger !== null && sessionId.length > 0) {
      ...
      if (claim?.ok !== true) {
        ...
        const cap = claim?.cap ?? '?'
        const result = fail(role.name, `attempt budget spent for role "${role.name}" this session (${used}/${cap}) — no consultation was started`, 'budget', { source })
        statusFinish(entry, 'failed', { failure: result.failure, error: result.error })
        return result
      }
      reservation = claim
    }
    ...
    if (slot.ok !== true) {
      ...
      const result = fail(role.name, 'consultation cancelled while waiting for a free consultation slot', 'concurrency', { source, aborted: true })
      statusFinish(entry, 'aborted', { failure: result.failure, error: result.error })
      return result
    }

    statusUpdate(entry, { phase: 'running' })
    ...
    let outcome = 'pending'
    try {
      return await run({ role, question, req, source, signal, picked, effort, entry, mark: (o) => { outcome = o } })
    } finally {
      // A run that threw before classifying itself still closes its card.
      statusFinish(entry, signal?.aborted === true ? 'aborted' : 'failed', {})
      watch.release()
      if (reservation !== null) {
  ...
  async function run({ role, question, req, source, signal, picked, effort, entry, mark }) {
    const primary = picked.primary
    const fallbackModel = picked.fallbackModel

    const budgetUsd = Number(settings.maxBudgetUsd)
    const systemPrompt = buildSystemPrompt(role, workspace)
    const outputSchema = envelopeSchemaForRole(role)
    ...
    const invoke = async (model) => {
      const options = {
        ...
        systemPrompt,
        outputSchema,
        maxTurns: source === 'test' ? TEST_MAX_TURNS : positiveInt(settings.maxTurns, 16),
        ...
        ...(entry !== null && entry !== undefined
          ? {
              onProgress(event) {
                if (event.type === 'spawn') statusUpdate(entry, { meta: { pid: event.pid } })
                else if (event.type === 'stdout') statusUpdate(entry, { meta: { outputBytes: event.totalBytes } })
                else if (event.type === 'exit') statusUpdate(entry, { meta: { exitCode: event.code, exitSignal: event.signal } })
              },
            }
          : {}),
      }
    ...
    if (eligible) {
      ...
      statusUpdate(entry, { phase: 'fallback' })
    ...
    const meta = {
      ...result.meta,
      effectiveModel: result.meta?.actualModel || effectiveModel,
      effectiveEffort: result.meta?.actualEffort || effort,
    ...
    const statusMeta = {
      ...
      cliDurationMs: meta.durationMs,
      costUsd: meta.costUsd,
      cliSessionId: meta.sessionId,
      rejectedArgs: meta.rejectedArgs,
    }
    if (!result.ok) {
      statusFinish(entry, failure === 'aborted' ? 'aborted' : 'failed', {
        failure,
        error: result.error,
        model: { effective: meta.effectiveModel, usedFallback },
        effort,
        meta: statusMeta,
      })
      return { ok: false, role: role.name, error: result.error, failure, meta }
    }
    const attached = attachEnvelope({ ok: true, answer: result.answer, meta }, packet.overflow, role)
    statusFinish(entry, 'succeeded', {
      answer: attached.answer,
      envelopeStatus: attached.meta.envelopeStatus ?? null,
      model: { effective: attached.meta.effectiveModel, usedFallback },
      effort: attached.meta.effectiveEffort ?? effort,
      meta: statusMeta,
    })
