 ...
 * expert sees evidence as evidence, not as instructions. Only objective,
 * question, success criteria and constraints carry task authority. Everything
 * describing the attempted work or its observations remains untrusted input.
 * Overflow is recorded rather than silently dropped.
 */

/** Default packet character budget (the maxBytes name is retained for compatibility). */
...
export function buildConsultationPacket(input = {}) {
  ...
  pushTrusted('success-criteria', input.successCriteria)
  pushTrusted('constraints', input.constraints)
  pushUntrusted('current-attempt', input.currentAttempt)
  const artifacts = asText(input.artifacts).trim().length > 0 ? input.artifacts : input.context
  pushUntrusted('artifacts', artifacts)
  pushUntrusted('verification', input.verification)
  pushUntrusted('unknowns', input.unknowns)

  // This is deliberately a JavaScript-character budget, not a UTF-8 byte
  // limit. The legacy maxBytes input name cannot be changed without breaking
  // callers; overflow diagnostics use the accurate unit.
  const headerChars = sections.reduce((sum, section) => (
    sum + section.label.length + (section.untrusted ? UNTRUSTED.length + 8 : 4) + 8
  ), 0)
  let remaining = Math.max(256, limit - headerChars)
  const parts = []
  ...
  for (const section of sections) {
    ...
    if (clipped.truncated) overflow.push(`${section.label} truncated to ${share} characters`)
