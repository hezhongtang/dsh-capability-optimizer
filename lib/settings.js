...
import {
  DEFAULT_TOP_TIER_CONSULTANT_MODEL,
  canonicalizeConsultantModel,
  isHighIntellectRole,
  pinConsultantModel,
  TOP_TIER_CONSULTANT_MODELS,
...
function defaultRoleFrom(role) {
  return {
    ...
    model: isHighIntellectRole(role.name) ? DEFAULT_TOP_TIER_CONSULTANT_MODEL : '',
    fallbackModel: '',
    effort: '',
    enabled: true,
  }
...
export function validateBackendSettings(candidate) {
  ...
  if (!Array.isArray(candidate.roles) || candidate.roles.length === 0) {
    ...
    for (const [index, entry] of candidate.roles.entries()) {
      ...
      let model = nonEmptyString(entry.model)
      let fallbackModel = nonEmptyString(entry.fallbackModel)
      if (isHighIntellectRole(name) && enabled) {
        const classified = canonicalizeConsultantModel(model)
        if (classified.action === 'empty') {
          // Older files left advisor on the global default. Pin the top-tier
          // id so a weaker CLI default cannot silently become the advice-giver.
          model = DEFAULT_TOP_TIER_CONSULTANT_MODEL
        } else if (classified.action === 'upgrade') {
          model = classified.model
        } else if (classified.action === 'reject') {
          problems.push(
            `roles[${index}] ("${name}"): advisor must use a top-tier model ` +
            `(${[...TOP_TIER_CONSULTANT_MODELS].join(', ')}), not ${model}`,
          )
        }
        const fallback = canonicalizeConsultantModel(fallbackModel)
        if (fallback.action === 'upgrade') {
          fallbackModel = fallback.model
        } else if (fallback.action === 'reject') {
          problems.push(
            `roles[${index}] ("${name}"): advisor fallback must use a top-tier model ` +
            `(${[...TOP_TIER_CONSULTANT_MODELS].join(', ')}), not ${fallbackModel}`,
          )
        }
      }
      roles.push({
...
/**
 * Repair advisor pin drift in a file document without voiding the rest of it.
 * Aliases upgrade; weaker models pin to the versioned top-tier id; a weaker
 * advisor fallback is dropped so a load cannot hop to haiku.
 */
export function repairSettingsDocument(fileSettings) {
  const v2 = toV2(fileSettings)
  const repairs = []
  const active = v2.backends[ACTIVE_BACKEND]
  if (active === null || typeof active !== 'object' || Array.isArray(active) || !Array.isArray(active.roles)) {
    return { document: v2, repairs }
  }
  const roles = active.roles.map((entry, index) => {
    if (entry === null || typeof entry !== 'object') return entry
    const name = slugifyRoleName(entry.name ?? '')
    if (!isHighIntellectRole(name) || entry.enabled === false) return entry
    const next = { ...entry }
    const pinned = pinConsultantModel(entry.model)
    if (pinned.action !== 'keep') {
      repairs.push({
        path: `roles[${index}].model`,
        from: typeof entry.model === 'string' ? entry.model : '',
        to: pinned.model,
      })
      next.model = pinned.model
    }
    if (typeof entry.fallbackModel === 'string' && entry.fallbackModel.trim().length > 0) {
      const fallback = canonicalizeConsultantModel(entry.fallbackModel)
      if (fallback.action === 'upgrade') {
        repairs.push({ path: `roles[${index}].fallbackModel`, from: entry.fallbackModel, to: fallback.model })
        next.fallbackModel = fallback.model
      } else if (fallback.action === 'reject') {
        repairs.push({ path: `roles[${index}].fallbackModel`, from: entry.fallbackModel, to: '' })
        next.fallbackModel = ''
      }
    }
    return next
  ...
  return {
    document: { ...v2, backends: { ...v2.backends, [ACTIVE_BACKEND]: { ...active, roles } } },
    repairs,
  }
}

function rowRoleModel(name, raw) {
  const trimmed = typeof raw === 'string' ? raw.trim() : ''
  if (!isHighIntellectRole(name)) return trimmed
  return pinConsultantModel(trimmed).model
}

function rowRoleFallback(name, raw) {
  const trimmed = typeof raw === 'string' ? raw.trim() : ''
  if (!isHighIntellectRole(name) || trimmed.length === 0) return trimmed
  const classified = canonicalizeConsultantModel(trimmed)
  return classified.action === 'keep' || classified.action === 'upgrade' ? classified.model : ''
}
  ...
  if (rowConfig.roles.length > 0) {
    ...
    for (const entry of rowConfig.roles) {
      ...
      merged.set(name, {
        ...
        description: typeof entry.description === 'string' && entry.description.trim().length > 0 ? entry.description.trim() : `Custom expert role "${name}".`,
        systemPrompt,
        model: rowRoleModel(name, entry.model),
        fallbackModel: rowRoleFallback(name, entry.fallbackModel),
 ...
 * An advisor pin repair never voids the rest of the file; unrecoverable
 * schema problems still fall back to row config, but the problems travel
