 ...
 * GET  /consultation-status — one session's live consultation dock entries.
 */
...
/**
 * Origin gate for read-only GET routes. Real same-origin browser fetches do
 * not always send `Origin` on GET, so its absence is accepted; a present,
 * unparseable or mismatched origin is refused. No CORS headers are emitted,
 * so a cross-site reader still cannot read the body.
 */
 ...
 * @param {object} runtime - shared mutable runtime: { rowConfig, loadFileSettings, apply, autoRuntime, consultationStatus? }.
...
export function mountOptimizerRoutes(host, runtime) {
  ...
  disposers.push(host.webServer.register({
    kind: 'exact',
    path: '/dsh-capability-optimizer/consultation-status',
    handler: async (request, response) => {
      if (request.method !== 'GET') {
        response.writeHead(405, { allow: 'GET' })
        response.end()
      ...
      try {
        const sessionId = new URL(request.url, 'http://localhost').searchParams.get('session')?.trim() ?? ''
        if (sessionId.length === 0) {
          sendJson(response, 400, { error: 'missing session' })
          return
        }
        const store = runtime.consultationStatus
        if (store === null || store === undefined || typeof store.snapshot !== 'function') {
          sendJson(response, 503, { error: 'status store unavailable' })
          return
        }
        sendJson(response, 200, {
          session: sessionId,
          now: Date.now(),
          entries: store.snapshot(sessionId),
        })
      } catch (error) {
        sendJson(response, 500, { error: error instanceof Error ? error.message : String(error) })
      }
