...
const ROLE_WHEN = {
  ...
  designer: 'after the first file write of a turn, on the next step',
}
...
function newTurn(successBase) {
  return { wrote: false, attempted: new Set(), nudged: new Set(), successBase, refusedWrite: false }
}
...
export function createAutoConsultRuntime({ getDefaults, getRoster } = {}) {
  ...
  /** `off | remind | required`. Unknown values degrade to remind. */
  function mode() {
    const raw = getDefaults()?.mode
    return raw === 'off' || raw === 'required' ? raw : 'remind'
  }

  function succeededAnyThisTurn(sessionId) {
    const entry = sessions.get(sessionId)
    if (entry === undefined) return false
    const usage = ledger.usage(sessionId)
    for (const [role, tally] of Object.entries(usage)) {
      if (tally.succeeded > (entry.turn.successBase[role] ?? 0)) return true
    }
    return false
  }
  ...
  return {
    ...
    /**
     * Opt-in `required` gate: compute a refusal for the first write-family
     * tool of a turn until a consultation has succeeded this turn. One
     * computed refusal only; later writes pass. `remind` and `off` never
     * refuse. The host currently has no pre-execute hook, so the wiring
     * logs this and still observes the write.
     * @returns {{ refuse: boolean, reason?: string }}
     */
    refuseWrite(sessionId, tool) {
      if (mode() !== 'required') return { refuse: false }
      if (!isWriteTool(tool)) return { refuse: false }
      if (succeededAnyThisTurn(sessionId)) return { refuse: false }
      const entry = state(sessionId)
      if (entry.turn.refusedWrite === true) return { refuse: false }
      entry.turn.refusedWrite = true
      return {
        refuse: true,
        reason: 'required consultation: consult an enabled expert once this turn before writing',
      }
    },
  ...
  disposers.push(ctx.on('session/event', (session, event) => {
    ...
    if (event?.type === 'turn/start') runtime.onTurnStart(sessionId)
      ...
      if (name === 'consult_expert' || name === 'consult_panel') {
        ...
        const block = runtime.refuseWrite(sessionId, name)
        if (block.refuse) log('info', block.reason)
...
function modeGloss(value) {
  if (value === 'required') {
    return ' (harder reminder: consult before writing. The host cannot intercept write tools; a missed consult is logged, not blocked)'
  }
