...
export function createAutoConsultRuntime({ getDefaults, getRoster } = {}) {
  ...
  return {
    ...
    /**
     * Opt-in `required` gate: refuse the first write-family tool of a turn
     * until a consultation has succeeded this turn. One refusal only; later
     * writes pass. `remind` and `off` never refuse.
    ...
    policyText(sessionId) {
      ...
      const lines = [
        ...
        `- Auto-consult mode: ${mode()} (default remind; required refuses one write until a consult succeeds this turn).`,
