 ...
 * `exec.agent.id` is the session identity the ledger bills — absent for a
 * non-agent dispatch, in which case the call runs unbudgeted rather than
 * billing a bucket that is not the caller's.
 *
...
function sessionIdOf(exec) {
  ...
  return typeof viaSession === 'string' ? viaSession : ''
}
...
export async function registerConsultTools(tools, settings, ctx = null, deps = {}) {
  ...
  disposers.push(tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      return result.ok
        ? { ok: true, role: result.role, answer: result.answer, meta: result.meta }
